...
Request
   ...
   ├─► [RateLimiter]   (route group içinde — Grup A/B/C bazlı, IP başına)
...
|---|---|---|---|
| A | `/api/v1/quotes/*`, `/history/*`, `/ohlcv/*`, `/compare/*` | **30 RPS** per-IP | `rl:groupA:{ip}` |
| B | `/api/v1/orderbook/*`, `/spread/*`, `/funding/*`, `/funding-rate/*`, `/slippage/*`, `/liquidity/*`, `/efficiency/*`, `/rsi/*` | **5 RPS** per-IP | `rl:groupB:{ip}` |
| C | `/api/v1/whale-alerts`, `/wallet/*`, `/news`, `/ico-calendar`, `/etf-flows`, `/fees`, `/all-in-cost/*` | **2 RPS** per-IP | `rl:groupC:{ip}` |
...
### 3.7 Health Aggregation (`internal/handler/health.go`)
...
| `RATE_LIMIT_GROUP_B` | `5` | Grup B rate limit (orderbook/spread/funding/slippage/liquidity/efficiency) — RPS per-IP |
| `RATE_LIMIT_GROUP_C` | `2` | Grup C rate limit (whale-alerts/wallet/news/ico-calendar) — RPS per-IP |
...
| 3 | Distributed tracing yok | Açık | Jaeger / OpenTelemetry entegrasyonu |

---

*Tresaurio Platform — api-gateway v0.8.0*
